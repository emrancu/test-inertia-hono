...
__name(prodEntryTags, "prodEntryTags");
...
__name(inertiaMiddleware, "inertiaMiddleware");
function inertiaRender(c, component, props = {}) {
  ...
  const html = APP_HTML.replace("%INERTIA_PAGE%", JSON.stringify(page).replace(/</g, "\\u003c")).replace("%ASSET_TAGS%", assetTagsFor(component)).replace("%INERTIA_HEAD%", "");
  return c.html(html);
}
__name(inertiaRender, "inertiaRender");
function inertiaRedirect(c, location, status = 303) {
  return c.body(null, status, { "X-Inertia": "true", "Location": location });
}
__name(inertiaRedirect, "inertiaRedirect");
var app = new Hono2();
app.use("*", inertiaMiddleware());
app.get("/assets/*", module({ root: "" }));
...
// .wrangler/tmp/bundle-z2vngC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-z2vngC/middleware-loader.entry.ts
