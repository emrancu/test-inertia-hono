...
import { App, Request } from "../../src/core";
...
export class AuthManager {
	...
	private async regenerateSession() {
		if (Session.getSession().id) {
			...
			const sessionData = Session.rawData();
	...
	public async login(user: AuthUser, remember = false): Promise<void> {
		...
		Session.set(key, { guard: this.guardsName, userId: user.id });

		App.addToCurrentState("authUser", await this.formatUserData(user));

		Request.getContext().header(
			"set-cookie",
			await Session.getStorage().commitSession(Session.getSession(), {
